...
def _create_compilable_bucketed_decode_attn(num_seqs: int, num_blocks: int):
    """Bucketed decode-only kernel factory. Closes over num_seqs and num_blocks;
    inputs are pre-gathered / pre-padded / pre-masked by _run_bucketed_decode_dispatch.

    Spyre constraints: per-block Python list (not .select) so the block index
    stays at Dynamo-trace time (torch-spyre would emit Mod(d0, num_blocks) sticks
    otherwise); (num_seqs, num_kv_heads) folded into one leading axis so the
    matmul stays 4-D (lower_bmm rejects 5-D).
    """

    def specialized_bucketed_decode_kernel(
        q, k_pages_per_block, v_pages_per_block, mask_tiles, scale
    ):
        # q: [num_seqs * KV, QPK, 1, D]; k/v_pages_per_block[i]: same first three
        # dims with block_size in place of 1; mask_tiles[i]: [num_seqs*KV, 1, 1, block_size].
        tile_max = None
        tile_sum = None
        tile_output = None
...
class SpyreAttentionMetadataBuilder(AttentionMetadataBuilder[SpyreAttentionMetadata]):
    ...
    def _pad_num_blocks(self, num_blocks: int, max_blocks_available: int) -> int:
        ...
        if num_blocks == 0:
            ...
            # output, whereas a fully-masked padded tile divides by a zero
            # softmax denominator.
            return 0
    ...
    def build(
        ...
        if self.sliding_window is None:
            ...
            # need no special construction — kv_valid = kv_pos < seq_lens already
            # emits finfo.min for every column at or past the true length.
            ...
            # Left unpadded (padded_num_blocks stays None). len(active_bs) here is
            # a window-width quantity, nearly constant across decode steps, so
        ...
        if max_query_len == 1 and self.sliding_window is None and num_seqs >= _MIN_SEQS_BUCKET:
            ...
            if b_seqs is not None and b_blocks is not None:
                ...
                # Flat-from-birth (not [B_seqs, B_blocks].reshape(-1)): reshaping
                # a 2D tensor whose inner dim is narrower than the stick width
                # (32) emits a Mod(d0, b_blocks) stick expression the Spyre
                # inductor rejects.
                block_ids_padded_cpu = torch.zeros(b_seqs * b_blocks, dtype=torch.int32)
                for s, n in enumerate(real_num_blocks):
                    n_use = min(n, b_blocks)
                    block_ids_padded_cpu[s * b_blocks : s * b_blocks + n_use] = block_table[
                        s, :n_use
                    ]
...
class SpyreAttentionImpl(AttentionImpl[SpyreAttentionMetadata]):
    ...
    def _get_bucketed_decode_kernel(self, bucket_num_seqs: int, bucket_num_blocks: int):
        key = (bucket_num_seqs, bucket_num_blocks)
        if key not in self._decode_fns:
            self._decode_fns[key] = _maybe_compile(
                _create_compilable_bucketed_decode_attn(bucket_num_seqs, bucket_num_blocks),
                self._compile_attn,
            )
    ...
    def record_bucketed_decode_graphs(
        ...
        Separate from ``record_graphs``: the bucketed decode kernel
        (``_get_bucketed_decode_kernel`` / ``self._decode_fns``) has its own
        cache key and shape contract, unrelated to the varlen kernel's. It
        takes pre-gathered K/V/mask lists rather than raw pages, so recording
        it needs no real KV cache — dummy zero tensors of the right shape are
        enough to trace.
        """
        if not self._compile_attn:
            return 0
        if not envs.SPYRE_BUCKETED_DECODE:
            return 0
        # Mirrors _bucketed_decode_preconditions_met: the bucketed kernel has
        # no ALiBi or soft-cap support, so those layers never dispatch here.
        ...
        try:
            recorded = self._record_all_bucketed_decode(variants, bucketer.block_size, device)
        finally:
            torch._dynamo.config.accumulated_recompile_limit = prev_limit  # ty: ignore[invalid-assignment]

        logger.info(
            "Recorded %d/%d bucketed decode variants in %.2fs.",
            recorded,
            len(variants),
            time.time() - t_start,
        )
    ...
    def _record_all_bucketed_decode(
        self, variants: list[tuple[int, int]], block_size: int, device: torch.device
    ) -> int:
        recorded = 0
        for b_seqs, b_blocks in variants:
            key = (b_seqs, b_blocks)
            if key in self._decode_fns:
                continue
            t0 = time.time()
            try:
                self._record_one_bucketed_decode(b_seqs, b_blocks, block_size, device)
            except Exception:
                self._decode_fns.pop(key, None)
                logger.warning(
                    "Bucketed decode variant %s failed to record; it will "
                    "compile on first use instead.",
                    key,
                    exc_info=True,
                )
                continue
            recorded += 1
            logger.info("  recorded bucketed decode %s in %.2fs", key, time.time() - t0)
        return recorded

    def _record_one_bucketed_decode(
        self, b_seqs: int, b_blocks: int, block_size: int, device: torch.device
    ) -> None:
        """Trace one (b_seqs, b_blocks) variant on dummy args matching
        ``specialized_bucketed_decode_kernel``'s shape contract.
        """
        ...
        q = convert(
            torch.zeros(
                b_seqs * num_kv_heads, num_queries_per_kv, 1, head_size, dtype=self.model_dtype
            ),
            device=device,
        )
        k_list = [
        ...
        # All-zero (not -inf) additive tiles: the kernel does not gate on kv_len
        # here, and a zero mask cannot produce a fully-masked row (no NaN risk).
        mask_list = [
            convert(
                torch.zeros(b_seqs * num_kv_heads, 1, 1, block_size, dtype=self.model_dtype),
                device=device,
            )
            for _ in range(b_blocks)
        ]
        kernel(q, k_list, v_list, mask_list, self.scale)
    ...
    def _run_bucketed_decode_dispatch(
        ...
        # one leading axis so lower_bmm sees ≤ 4-D; (3) K/V/mask arrive as
        # Python lists indexed at Dynamo-trace time (torch-spyre would emit
        # Mod(d0, num_blocks) for runtime .select); (4) result scatter is a
        ...
        q_packed = (
            ...
            .reshape(b_seqs * num_kv_heads, num_queries_per_kv, 1, head_size)
            .clone()
        )
    ...
    def _online_softmax_attention(
        ...
        for seq_idx in range(num_seqs):
            ...
            # Ties the builder's tile count to the kernel's num_blocks key: a
            # short slice here would silently hand the kernel a wrong shape.
            ...
            #
            # Padded blocks get a tile too (the loop iterates active_bs), whose
            # values stay finite — slopes are small negative powers of two — so
            # they saturate under the mask's finfo.min and stay inert. Only +inf
            # could corrupt the running max, and it is unreachable here.
            #
