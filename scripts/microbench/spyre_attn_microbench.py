...
def record_padding(row, attn_metadata, query_lens, seq_lens, block_size, sliding_window=None):
    ...
    if sliding_window is not None:
        # A window never iterates every block of the sequence: it drops the blocks
        # outside the earliest query's window, then pads back up to the most its
        # bucket can produce. So the identity is the wrong expectation here -- the
        # bound is all that is meaningful, and it holds per sequence.
