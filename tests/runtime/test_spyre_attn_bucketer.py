...
class TestVariants:
    ...
    def test_num_seqs_buckets_empty_below_min_batched(self, max_num_seqs, monkeypatch):
        """max_num_seqs < _MIN_BATCHED_SEQS can never reach the batched path, so
        there is nothing to record -- clamping down to max_num_seqs would enumerate
        a variant build() declines, failing every recording attempt. An explicit
        override does not resurrect it either."""
