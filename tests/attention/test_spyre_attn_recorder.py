...
class TestRecordBatchedDecode:
    ...
    def test_window_variants_over_the_requested_budget_still_record(
        ...
        in the serving path. The realized axis is not simply smaller: the window
        drops out-of-window blocks, then build() pads back up to the bucket's own
        maximum, so only what build() returns predicts the gather.
        """
        bucketer = sliding_window_builder._attn_bucketer = make_bucketer()
        # Sized so the gap exists: wide enough for what the window actually
        # realizes, too narrow for what the buckets ask for. NUM_PAGES itself no
        # longer straddles any variant, since padding lifts the realized axis back
        # up to the bucket maximum.
        pages = 12
        ...
        # Over the budget as requested. Whether the realized gather fits is a
        # question about build()'s output, so ask build() rather than the bucket.
        reachable = []
