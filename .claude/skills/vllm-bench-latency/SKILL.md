...
**Token count** — the platform builds one graph per entry in `compile_sizes`. The default is **already sparse**: powers of two from 1 up to `--max-num-seqs`, plus one prefill bucket at `min(max_num_batched_tokens, 512)` (`platform.py` `apply_config_platform_defaults`). Overriding it pays off only when the benchmark runs *fewer* concurrent sequences than `--max-num-seqs` allows (a serving default of 256 gives 10 buckets; `--max-concurrency 4` needs 4), or to pin the ladder so a later `--max-num-seqs` / `--max-model-len` change cannot move it. **Read the real ladder before assuming there is anything to trim** — it is printed verbatim in the `core.py:121 Initializing a V1 LLM engine ... with config:` dump (one enormous line; grep it for `compile_sizes`). The `Warming up model with <N> bucket sizes` line gives only the count, min and max.
...
CSIZES='[1,2,4,512]'   # at bs=4/512 this IS the default -- pins it, saves nothing
```
...
**When the crossings are the point, pay for them at warmup.** The checked-in suite deliberately breaks the second rule: it runs the trace's real decode length (`out576`, crossing 5 boundaries) because a decode capped at one block would not measure what the traces actually do. That is only safe because `bench latency` warms the *identical* shape — every crossing is recorded during the warmup iterations and reused by the timed ones — so `--num-iters-warmup ≥ 2` is load-bearing there, not hygiene, and the step-3 compile-leak gate is what confirms nothing leaked. Do not carry `out576` over to a one-off run of a *different* shape without warming it the same way.
